(function () {
    ...
    function qs(sel, root) {
        try {
            ...
            return null; // invalid selector protection
        }
    }
    ...
    function removeAllButLast(selector) {
    ...
    function insertAfter(refNode, htmlString) {
    ...
    function getCountry() {
        try {
            if (
                ...
                if (Shopify.routes.root === '/') return 'country=' + Shopify.country.split('-')[0];
                return 'language=' + (Shopify.locale ? Shopify.locale.split('-')[0] : '');
            }
        } catch (e) {}
        return "";
    }

    // JSONP helper (for extreme-ip-lookup)
    function jsonp(url, callbackParam, timeoutMs) {
        ...
        return new Promise(function (resolve, reject) {
            var cbName = '__areviews_jsonp_cb_' + Math.random().toString(36).slice(2);
            ...
            script.src = url + sep + encodeURIComponent(callbackParam) + '=' + encodeURIComponent(cbName);
            ...
            // guard head
            var head = document.head || qs('head') || document.documentElement;
    ...
    var areviews_regex = crawlerAgentRegex; // already regex

    // -----------------------------
    // Globals (match old behavior)
    // -----------------------------
    if (typeof window.customer_id === 'undefined') window.customer_id = '';
    if (typeof window.checkDuplication === 'undefined') window.checkDuplication = false;
    if (typeof window.counter === 'undefined') window.counter = 0;

    // -----------------------------
    // Skeleton functions rewritten
    // -----------------------------
    function createHeaderSkeleton() {
        return ' <div class="areviews_skeleton_header"> <div class="areviews_skeleton_header_content"> <div class="areviews_skeleton_header_left"> <div class="areviews_skeleton_title"></div> <div class="areviews_skeleton_rating_wrapper"> <div class="areviews_skeleton_rating_stars"></div> <div class="areviews_skeleton_rating_text"></div> <div class="areviews_skeleton_rating_count"></div> </div> </div> <div class="areviews_skeleton_header_right"> <div class="areviews_skeleton_write_button"></div> </div> </div> <div class="areviews_skeleton_filters"> <div class="areviews_skeleton_filter_item"></div> <div class="areviews_skeleton_filter_item"></div> <div class="areviews_skeleton_filter_item"></div> <div class="areviews_skeleton_filter_item"></div> </div> </div> ';
    }
    ...
    function createGridSkeleton(count) {
        ...
        for (var i = 0; i < count; i++) {
            var hasImage = Math.random() > 0.3;
            var textLines = Math.floor(Math.random() * 3) + 2;
            skeletonHTML += ' <div class="large-3 cell item areviews_skeleton_item_grid"> <div class="areviews_skeleton_card_grid"> <div class="areviews_skeleton_header_grid"> <div class="areviews_skeleton_info_grid"> <div class="areviews_skeleton_name_grid"></div> <div class="areviews_skeleton_stars_grid"></div> </div> <div class="areviews_skeleton_avatar_grid"></div> </div> <div class="areviews_skeleton_content_grid">';
            for (var j = 0; j < textLines; j++) skeletonHTML += '<div class="areviews_skeleton_text_grid"></div>';
            skeletonHTML += '</div>';
            if (hasImage) skeletonHTML += '<div class="areviews_skeleton_image_block_grid"></div>';
            skeletonHTML += ' <div class="areviews_skeleton_date_grid"></div> </div> </div> ';
        }
        return skeletonHTML;
    }

    function createSkeletonLoader(count) {
        return ' <div class="areviews_skeleton_wrapper"> ' +
            createHeaderSkeleton() +
            ' <div class="areviews_skeleton_container_grid">' + createGridSkeleton(count) + '</div> </div> ';
    }

    function showSkeletonLoading(count) {
        var az = qs('#az_reviews');
        if (!az) return;
        az.innerHTML = createSkeletonLoader(count);
    }

    function hideSkeletonLoading() {
        var az = qs('#az_reviews');
        if (!az) return;
        var wrap = qs('.areviews_skeleton_wrapper', az);
        if (!wrap) return;
        if (wrap.classList) wrap.classList.add('fade-out');
        setTimeout(function () { if (wrap && wrap.remove) wrap.remove(); }, 300);
    }

    function createTitleRatingSkeleton() {
        return ' <div class="areviews_title_rating_skeleton"> <div class="areviews_title_rating_skeleton_inner"> <div class="areviews_title_skeleton_stars"></div> <div class="areviews_title_skeleton_score"></div> <div class="areviews_title_skeleton_text"></div> </div> </div> ';
    }


    function AreviewsLiveHTML(selector, content){
        ...
        elements.forEach(function (element, index) {
            var newContent = typeof content === 'function'
            ...
            // --- Step 1: parse into a detached fragment (jQuery's buildFragment) ---
            var temp = document.createElement('div');
            // jQuery wraps certain tags (tr, td, option…) — handle the common ones
            var wrapMap = {
                thead: [1, '<table>', '</table>'],
                tbody: [1, '<table>', '</table>'],
                tfoot: [1, '<table>', '</table>'],
            ...
            var tagMatch = newContent.match(/^[\s\S]*?<([a-zA-Z]+)/);
            var tag = tagMatch ? tagMatch[1].toLowerCase() : '';
            var wrap = wrapMap[tag];

            if (wrap) {
                temp.innerHTML = wrap[1] + newContent + wrap[2];
                var depth = wrap[0];
                while (depth--) temp = temp.firstChild;
            } else {
                temp.innerHTML = newContent;
            }

            // --- Step 2: extract scripts (jQuery's domManip) ---
            var scripts = Array.from(temp.querySelectorAll('script'));
            scripts.forEach(function (s) { s.parentNode.removeChild(s); });

            // --- Step 3: set clean HTML ---
            element.innerHTML = temp.innerHTML;

            // --- Step 4: execute scripts in order (jQuery's globalEval) ---
            domManip(scripts);
        });
    ...
    function domManip(scripts) {
        ...
        function next() {
            ...
            // Skip non-JS script types (templates, json, etc.)
            ...
            if (script.src) {
                ...
                var key = '_ar_' + script.src.replace(/[^a-zA-Z0-9]/g, '_');
                if (window[key]) {
                    next(); // already loaded
                    return;
                }
                ...
                // Copy all attributes (async, defer, crossorigin, etc.)
                Array.from(script.attributes).forEach(function (attr) {
                    newScript.setAttribute(attr.name, attr.value);
                });
                newScript.onload  = next;
                newScript.onerror = next; // skip broken scripts, don't block
                document.head.appendChild(newScript);

            } else {
                var code = script.textContent || script.innerText || '';
                if (code.trim()) {
                    // jQuery's globalEval: indirect eval runs in global scope
                    try {
                        (0, eval)(code);
                    } catch (e) {
                        // mirror jQuery: swallow, never block the chain
                    }
                }
                next();
            }
    ...
    // Avoid :is() for compatibility; your qs() catch prevents crashes, but this is more reliable.
    function firstHeaderItempropName() {
        var headers = qsa('h1[itemprop="name"],h2[itemprop="name"],h3[itemprop="name"],h4[itemprop="name"],h5[itemprop="name"],h6[itemprop="name"]');
        return headers.length ? headers[0] : null;
    }

    function showTitleRatingSkeleton() {
        var skeletonHTML = createTitleRatingSkeleton();

        var ref =
            qs('.areviews_header_stars') ||
            firstHeaderItempropName() ||
            qs('.product-title') ||
            qs('[class*=product][class*=title]');

        if (ref) insertAfter(ref, skeletonHTML);
    }

    function hideTitleRatingSkeleton() {
        var el = qs('.areviews_title_rating_skeleton');
        if (!el) return;
        if (el.classList) el.classList.add('fade-out');
        setTimeout(function () { if (el && el.remove) el.remove(); }, 300);
    }

    // -----------------------------
    // Insert rating block (vanilla)
    // -----------------------------
    function injectProductRating() {
        var productRatingEl = qs('.Products_reviews');
        if (!productRatingEl) return;

        var product_rating = productRatingEl.innerHTML || '';
        var html = '<div style="text-decoration: none!important;" class="areviews_main_header_container" href="javascript:void(0);">' +
            product_rating +
            '</div>';
        ...
        if (anchor) insertAfter(anchor, html);
    }

    // -----------------------------
    // XHR load reviews (same endpoint)
    // -----------------------------
    function loadReviews() {
        var az = qs('#az_reviews');
        if (!az) return;

        var product_id = (az.dataset && az.dataset.product_id) ? az.dataset.product_id : '';
        if (!product_id) return;

        if (window.checkDuplication !== false) return;
        window.checkDuplication = true;

        showTitleRatingSkeleton();
        showSkeletonLoading(8);

        var XHR = window.OriginalXMLHttpRequest || window.XMLHttpRequest;
        if (!XHR) {
            hideTitleRatingSkeleton();
            hideSkeletonLoading();
            return;
        }

        var xhr;
        try { xhr = new XHR(); } catch (e) {
            hideTitleRatingSkeleton();
            hideSkeletonLoading();
            return;
        }
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                // az.innerHTML = xhr.responseText || '';
                AreviewsLiveHTML(az,xhr.responseText);
                injectProductRating();
                window.counter = 1;
            } else {
                qsa('.areviews_loading').forEach(function (el) { if (el && el.remove) el.remove(); });
                console.log('XHR Error:', xhr.status, xhr.statusText);
            }
        };

        var params = 'product_id=' + encodeURIComponent(product_id) +
            '&customer_id=' + encodeURIComponent(window.customer_id || '') +
            '&customer_country=' + encodeURIComponent(getCountry());

        try { xhr.send(params); } catch (e) {
            hideTitleRatingSkeleton();
            hideSkeletonLoading();
        }
    }
    ...
    // -----------------------------
    // Send visitor data (no jQuery)
    // -----------------------------
    var AreviewsUserData;
    function sendVisitorData() {
        ...
        var os_full = 'Unknown';
        var device_class = 'Desktop';

        try {
            if (typeof UAParser !== 'undefined') {
                var parser = new UAParser();
                var result = parser.getResult ? parser.getResult() : {};
                browser_name = (result.browser && result.browser.name) || browser_name;
                browser_version = (result.browser && result.browser.version) || browser_version;

                var os_name = (result.os && result.os.name) || 'Unknown';
                var os_version = (result.os && result.os.version) || '';
                os_full = os_name + (os_version ? ' ' + os_version : '');

                var device_type = (result.device && result.device.type) || 'desktop';
                device_class = device_type ? (device_type.charAt(0).toUpperCase() + device_type.slice(1)) : device_class;
            }
        } catch (e) {}
        ...
        var ipPromise = jsonp('https://extreme-ip-lookup.com/json/?key=2RKRWkmXX1BhneujyJYn', 'callback', 3000)
            .catch(function () {
                // Fallback: fetch from your own server
                if (typeof fetch === 'undefined') return null;
                return fetch('https://areviewsapp.com/getUserLocation.php')
        ...
        ipPromise
            .then(function (ipData) {
                ...
                var shop = '';
                try { shop = (typeof Shopify !== 'undefined' && Shopify.shop) ? Shopify.shop : ''; } catch (e) {}

                var productId = '';
                try { productId = (az.dataset && az.dataset.product_id) ? az.dataset.product_id : ''; } catch (e) {}

                var bodyStr = null;
                try {
                    AreviewsUserData = ipData;
                    if (typeof URLSearchParams !== 'undefined') {
                        bodyStr = new URLSearchParams({
                            ...
                            ip: (ipData && ipData.query) ? ipData.query : '',
                            country_name: (ipData && ipData.country) ? ipData.country : '',
                            country_code: (ipData && ipData.countryCode) ? ipData.countryCode : '',
                            city: (ipData && ipData.city) ? ipData.city : ''
                        }).toString();
                    } else {
                        ...
                        bodyStr =
                            ...
                            '&customer_id=' + enc(window.customer_id || '') +
                            '&browser=' + enc(browser_name) +
                            '&version=' + enc(browser_version) +
                            '&device=' + enc(os_full) +
                            '&device_name=' + enc(device_class) +
                            '&referrer_type=' + enc(referrer_type) +
                            '&ip=' + enc((ipData && ipData.query) ? ipData.query : '') +
                            '&country_name=' + enc((ipData && ipData.country) ? ipData.country : '') +
                            '&country_code=' + enc((ipData && ipData.countryCode) ? ipData.countryCode : '') +
                            '&city=' + enc((ipData && ipData.city) ? ipData.city : '');
                    }
                } catch (e) {
                    return null;
                }
                ...
                try {
                    return fetch('https://phplaravel-1572712-6125647.cloudwaysapps.com/api/store_visitors', {
                        method: 'POST',
    ...
    // -----------------------------
    // Boot (run when DOM ready)
    // -----------------------------
    function boot() {
        try {
            if (areviews_regex.test(navigator.userAgent || '')) return;
        } catch (e) {
            // if navigator blocked for any reason, just continue
        }

        var az = qs('#az_reviews');
        if (!az) return;

        removeAllButLast('#az_reviews.az_reviews');

        loadReviews();
        sendVisitorData();
    }

    // Ensure DOM exists before searching for #az_reviews
...
var Areviews_app_js = function (jQuery) {
    ...
    if (!ApexInIframe()) {
        fetch(get_url() + ".js")
            ...
            .then(function (e) {
                ...
                var image_url = null;
                if (typeof e.image !== "undefined" && e.image !== null && typeof e.image.src !== "undefined") {
                    image_url = e.image.src;
                } else if (typeof e.featured_image !== "undefined") {
                    image_url = e.featured_image;
                }
            ...
            .catch(function (err) {
                console.warn("areviews: Failed to fetch product JSON:", err);
            });
    }
    ...
    function add_show_image_data(e) {
        var reviewsContainer = e.closest(".areviews_slide_image_details");
         ...
         if (!reviewsContainer) {
            console.warn("add_show_image_data: .areviews_slide_image_details not found for", e);
            return;
        }
